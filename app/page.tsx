...
export default function Home() {
  ...
  const [adminOpenPanel, setAdminOpenPanel] = useState<"calendar" | "bookings" | "customers" | null>(null);
  ...
  function loginAdmin(e: FormEvent<HTMLFormElement>) {
    ...
    if (adminPassword === "8685") {
      setAdminMode(true); setAdminLoginOpen(false); setAdminPassword(""); setAdminError(false); setSelectedDate(null); setAdminOpenPanel(null);
...
<img src="/hero-bathroom-steam-final.png" alt="열린 배수구에 강한 스팀을 분사하는 욕실 청소 장면" />
    ...
    <div className="about-left"><div className="portrait"><img className="profile-photo" src="/profile-won-sunhee.webp" alt="원선희 홈크린마스터" /><div className="nameplate"><small>YOUR CLEANER</small><b>홈크린마스터</b></div></div><div className="about-copy"><h2 className="visitor-title"><span>누가 방문하는지,</span><em>미리 확인하세요.</em></h2><blockquote>“낯선 작업자가 오는 불안 없이,<br /><span className="quote-indent">사진 속 제가 항상 방문합니다.”</span></blockquote></div></div>
        <div className="about-greeting" aria-label="인사말 영역">
          ...
          <p>이러한 경험을 바탕으로, 이제 루원시티·청라 지역에 특화된 주방·욕실 정기 관리 서비스를 새롭게 시작합니다.</p>
      ...
      {adminMode && <div className="admin-only-header shell"><strong>관리자 관리</strong><nav className="admin-menu" aria-label="관리자 메뉴"><button type="button" className={adminOpenPanel === "calendar" ? "active" : ""} onClick={() => setAdminOpenPanel(adminOpenPanel === "calendar" ? null : "calendar")}>예약달력 <span>{adminOpenPanel === "calendar" ? "▲" : "▼"}</span></button><button type="button" className={adminOpenPanel === "bookings" ? "active" : ""} onClick={() => setAdminOpenPanel(adminOpenPanel === "bookings" ? null : "bookings")}>예약 현황 <span>{adminOpenPanel === "bookings" ? "▲" : "▼"}</span></button><button type="button" className={adminOpenPanel === "customers" ? "active" : ""} onClick={() => setAdminOpenPanel(adminOpenPanel === "customers" ? null : "customers")}>고객정보 <span>{adminOpenPanel === "customers" ? "▲" : "▼"}</span></button></nav><button type="button" onClick={() => { setAdminMode(false); setSelectedDate(null); setAdminOpenPanel(null); }}>고객 화면으로 돌아가기</button></div>}
                    {(!adminMode || adminOpenPanel === "calendar") && <section className="booking section" id="booking"><div className="shell booking-grid">
