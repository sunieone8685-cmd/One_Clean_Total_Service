...
export default function Home() {
  ...
  async function refreshSlots() {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/booking_slots?select=booking_date,booking_time,source&booking_date=gte.${monthStart}&booking_date=lte.${monthEnd}&site=eq.one_clean`, {
  ...
  async function setBookingStatus(id: number, status: "예약접수" | "통화필요" | "예약확정" | "예약취소") {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/admin_set_booking_status_wonseonhee`, {
      method: "POST",
      headers: supabaseHeaders,
      body: JSON.stringify({ p_booking_id: id, p_status: status, p_password: "8685" }),
    });
    if (!response.ok || !(await response.json())) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    if (selectedDateKey && selectedTime) {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/reserve_booking`, {
        method: "POST",
        headers: supabaseHeaders,
        body: JSON.stringify({ p_date: selectedDateKey, p_time: selectedTime, p_name: name, p_phone: phone, p_service: service, p_address: address, p_site: "one_clean" }),
      });
      if (!response.ok) {
...
<img src="/hero-bathroom-steam-final.png" alt="열린 배수구에 강한 스팀을 분사하는 욕실 청소 장면" />
    ...
    <div className="about-left"><div className="portrait"><img className="profile-photo" src="/profile-won-sunhee.webp" alt="원선희 홈크린마스터" /><div className="nameplate"><small>YOUR CLEANER</small><b>홈크린마스터</b></div></div><div className="about-copy"><h2 className="visitor-title"><span>누가 방문하는지,</span><em>미리 확인하세요.</em></h2><blockquote>“낯선 작업자가 오는 불안 없이,<br /><span className="quote-indent">사진 속 제가 항상 방문합니다.”</span></blockquote></div></div>
      ...
      {adminMode && <div className="admin-only-header shell"><strong>관리자 관리</strong><nav className="admin-menu" aria-label="관리자 메뉴"><button type="button" className={adminSection === "calendar" ? "active" : ""} onClick={() => setAdminSection(adminSection === "calendar" ? null : "calendar")}>예약달력 <span>{adminSection === "calendar" ? "▲" : "▼"}</span></button><button type="button" className={adminSection === "customers" ? "active" : ""} onClick={() => setAdminSection(adminSection === "customers" ? null : "customers")}>고객정보 <span>{adminSection === "customers" ? "▲" : "▼"}</span></button></nav><button type="button" onClick={() => { setAdminMode(false); setSelectedDate(null); setAdminSection(null); }}>고객 화면으로 돌아가기</button></div>}
